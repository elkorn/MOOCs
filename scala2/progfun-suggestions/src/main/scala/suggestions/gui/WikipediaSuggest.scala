...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = Observable { observer =>
      val responses = searchTerms concatRecovered wikiSuggestResponseStream

      responses subscribe { x =>
        observer.onNext(x)
      }
    ...
    println("subbed to suggestionSubscription")
      ...
      button.clicks.subscribe {
        _ =>
          suggestionList.selection.items match {
            case list => list.foreach(item => observer.onNext(item))
          }
      }
    }

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = Observable { pagesObserver: Observer[Try[String]] =>
      selections.subscribe {
        term => wikiPageResponseStream(term).recovered.subscribe(pagesObserver)
      }
    }
    ...
    println("subbed to pageSubscription")
