...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      ...
      timeout.subscribe(x => subject.onCompleted(), err => subject.onCompleted(), () => subject.onCompleted())
      obs.subscribe(x => subject.onNext(x), err => subject.onError(err), () => subject.onCompleted())
      subject
    }
