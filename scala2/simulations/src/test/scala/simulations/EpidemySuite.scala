...
class EpidemySuite extends FunSuite {
  ...
  test("value bounding works correctly") {
    val es = new EpidemySimulator
    assert(es.bounded(8,8) === 0)
    assert(es.bounded(7,8) === 7)
    assert(es.bounded(9,8) === 1)
    assert(es.bounded(16,8) === 0)
    assert(es.bounded(-1, 8) === 1)
    assert(es.bounded(-8, 8) === 0)
    assert(es.bounded(-9, 8) === 1)
    assert(es.bounded(-12, 8) === 4)
  }
